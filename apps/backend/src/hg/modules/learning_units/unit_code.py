...
# Prefijo de Área opcional: `(<AREA>-)?<DIM>-L<n>-P<n>-<seq>`.
_UNIT_CODE_RE = re.compile(r"^(?:([A-Z]{2,3})-)?([A-Z]{2,3})-L(\d{1,2})-P(\d{1,2})-(\d{1,4})$")
...
def parse_unit_code(code: str) -> UnitCode | None:
    ...
    pillar=2, number=1)``. Sin Área (o ``GEN``) → ``area=None`` (general).
...
def format_unit_code(unit: UnitCode) -> str:
    ...
    return f"{prefix}{unit.dimension.upper()}-L{unit.level}-P{unit.pillar}-{unit.number:03d}"
